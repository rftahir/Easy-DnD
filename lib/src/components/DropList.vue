    ...
    <template v-if="dropIn && dropAllowed">
      <template v-if="reordering">
        <template v-if="hasReorderingFeedback">
          <slot
            name="item"
            v-for="(item, index) in itemsBeforeReorderingFeedback"
            :item="item"
            :index="index"
          />
          ...
          <slot
            name="item"
            v-for="(item, index) in itemsAfterReorderingFeedback"
            :item="item"
            :index="index"
          />
        </template>
...
export default class DropList extends DropMixin {
  ...
  get dropAllowed() {
    if (this.dragInProgress) {
      if (this.reordering) {
        ...
        let superDropAllowed = DropMixin[
          "options"
        ].computed.dropAllowed.get.call(this);
        if (!superDropAllowed) {
          return false;
        } else {
  ...
  candidate(type, data, source): boolean {
    let superCandidate = DropMixin["options"].methods.candidate.call(
      this,
      ...arguments
    );
    return (
  ...
  computeForbiddenKeys() {
    let vnodes = this.noAnimations
      ? []
      : this.$children[0].$vnode.context.$children[0].$slots.default;
    return vnodes
      .map((vn) => vn.key)
      .filter(
        (k) => k !== undefined && k !== "drag-image" && k !== "drag-feedback"
      );
  }
